export type AgreementCategory =
  | "industry"
  | "construction"
  | "electrical"
  | "plumbing"
  | "inactive";

export type RateValidationStatus =
  | "missing_pdf"
  | "pdf_uploaded"
  | "pending_validation"
  | "validated";

export type CollectiveAgreement = {
  id: string;
  name: string;
  category: AgreementCategory;
  industryArea: string;
  isActive: boolean;
  supportsLocalAgreement: boolean;
  requiresUploadedAgreementPdf: boolean;
  pdfUrl?: string;
  pdfFileName?: string;
  pdfUploadedAt?: string;
  rateValidationStatus: RateValidationStatus;
  note?: string;
};

const localArchiveNote =
  "Der findes en privat lokal arkivkopi, men dokumentidentitet, paragraf, side og beregningsregel er ikke valideret. Kræver manuel validering.";

export const DEFAULT_COLLECTIVE_AGREEMENT_ID = "manual-review-required";

export const SUPPORTED_COLLECTIVE_AGREEMENT_IDS = [
  "manual-review-required",
  "industriens-overenskomst",
  "industri-trae-moebeloverenskomsten",
  "trae-moebeloverenskomsten",
  "industrioverenskomsten-byggeri",
  "bygningsoverenskomsten",
  "bygge-anlaegsoverenskomsten",
  "bygge-anlaegsoverenskomsten-dansk-haandvaerk-3f",
] as const;

const supportedCollectiveAgreementIds = new Set<string>(SUPPORTED_COLLECTIVE_AGREEMENT_IDS);

export function isSupportedCollectiveAgreementId(id: string) {
  return supportedCollectiveAgreementIds.has(id);
}

export function normalizeCollectiveAgreementId(id?: string | null) {
  return id && isSupportedCollectiveAgreementId(id) ? id : DEFAULT_COLLECTIVE_AGREEMENT_ID;
}

const allCollectiveAgreements: CollectiveAgreement[] = [
  {
    id: "manual-review-required",
    name: "Kræver manuel afklaring",
    category: "inactive",
    industryArea: "Ingen overenskomstversion er servervalideret",
    isActive: false,
    supportsLocalAgreement: false,
    requiresUploadedAgreementPdf: false,
    rateValidationStatus: "pending_validation",
    note: "Beregning og eksport er blokeret, indtil en administrator har tilknyttet en dokumenteret overenskomstversion.",
  },
  {
    id: "industriens-overenskomst",
    name: "Industriens Overenskomst",
    category: "industry",
    industryArea: "Industri / produktion / montage",
    isActive: true,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl:
      "https://www.co-industri.dk/sites/default/files/2025-08/Industriens-Overenskomst-2025-2028-2025_07_31.pdf",
    pdfFileName: "industriens-overenskomst.pdf",
    pdfUploadedAt: "2026-06-24",
    rateValidationStatus: "pdf_uploaded",
    note: "Dokumentidentiteten er hash-verificeret mod CO-industri 2025-2028, filversion 2025_07_31. Ingen satser eller tillæg er valideret til automatisk beregning.",
  },
  {
    id: "industri-trae-moebeloverenskomsten",
    name: "Industri-, Træ- og Møbeloverenskomsten",
    category: "industry",
    industryArea: "Industri / træ / møbel / produktion",
    isActive: true,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl: "",
    pdfFileName: "industri-trae-moebeloverenskomsten.pdf",
    pdfUploadedAt: "2026-06-24",
    rateValidationStatus: "pdf_uploaded",
    note: localArchiveNote,
  },
  {
    id: "trae-moebeloverenskomsten",
    name: "Træ- og Møbeloverenskomsten",
    category: "industry",
    industryArea: "Træ / møbel / produktion",
    isActive: true,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl: "",
    pdfFileName: "trae-moebeloverenskomsten.pdf",
    pdfUploadedAt: "2026-06-24",
    rateValidationStatus: "pdf_uploaded",
    note: localArchiveNote,
  },
  {
    id: "industrioverenskomsten-byggeri",
    name: "Industrioverenskomsten (Byggeri)",
    category: "construction",
    industryArea: "Byggeri / industri",
    isActive: true,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl: "",
    pdfFileName: "industrioverenskomsten-byggeri.pdf",
    pdfUploadedAt: "2026-06-24",
    rateValidationStatus: "pdf_uploaded",
    note: localArchiveNote,
  },
  {
    id: "industri-og-vaerkstedsoverenskomsten",
    name: "Industri- og Værkstedsoverenskomsten",
    category: "industry",
    industryArea: "Industri / værksted",
    isActive: true,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl: "",
    pdfFileName: "industri-vaerkstedsoverenskomsten.pdf",
    pdfUploadedAt: "2026-06-24",
    rateValidationStatus: "pdf_uploaded",
    note: localArchiveNote,
  },
  {
    id: "bygningsoverenskomsten",
    name: "Bygningsoverenskomsten",
    category: "construction",
    industryArea: "Byggeri / håndværk",
    isActive: true,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl:
      "https://www.3f.dk/-/media/files/artikler/overenskomst/byggegruppen/overenskomster/bygningsoverenskomsten-2025-2028_002.pdf",
    pdfFileName: "bygningsoverenskomsten.pdf",
    pdfUploadedAt: "2026-06-24",
    rateValidationStatus: "pdf_uploaded",
    note: "Dokumentidentiteten er hash-verificeret mod 3F's Bygningsoverenskomst 2025-2028. Ingen satser eller tillæg er valideret til automatisk beregning.",
  },
  {
    id: "bygge-anlaegsoverenskomsten",
    name: "Bygge- og Anlægsoverenskomsten",
    category: "construction",
    industryArea: "Byggeri / anlæg",
    isActive: true,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl:
      "https://www.3f.dk/-/media/files/artikler/overenskomst/byggegruppen/overenskomster/bygge--og-anlaegsoverenskomsten-2025-2028.pdf",
    pdfFileName: "bygge-anlaegsoverenskomsten.pdf",
    pdfUploadedAt: "2026-06-24",
    rateValidationStatus: "pdf_uploaded",
    note: "Dokumentidentiteten er hash-verificeret mod 3F's Bygge- og Anlægsoverenskomst 2025-2028. Ingen satser eller tillæg er valideret til automatisk beregning.",
  },
  {
    id: "bygge-anlaegsoverenskomsten-dansk-haandvaerk-3f",
    name: "Bygge- og Anlægsoverenskomsten (Dansk Håndværk / 3F)",
    category: "construction",
    industryArea: "Byggeri / anlæg / Dansk Håndværk",
    isActive: true,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl: "",
    pdfFileName: "bygge-anlaegsoverenskomsten-dansk-haandvaerk.pdf",
    pdfUploadedAt: "2026-06-24",
    rateValidationStatus: "pdf_uploaded",
    note: localArchiveNote,
  },
  {
    id: "jord-betonoverenskomsten",
    name: "Jord- og Betonoverenskomsten",
    category: "construction",
    industryArea: "Jord / beton / anlæg",
    isActive: true,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl: "",
    pdfFileName: "jord-betonoverenskomsten.pdf",
    pdfUploadedAt: "2026-06-24",
    rateValidationStatus: "pdf_uploaded",
    note: localArchiveNote,
  },
  {
    id: "murer-murerarbejdsmandsarbejde",
    name: "Murer- og murerarbejdsmandsarbejde",
    category: "construction",
    industryArea: "Murer / murerarbejdsmand",
    isActive: true,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl:
      "https://www.3f.dk/-/media/files/artikler/overenskomst/byggegruppen/overenskomster/murer--og-murerarbejdsmandsarbejde-dio-iii-2025-2028.pdf",
    pdfFileName: "murer-murerarbejdsmandsarbejde.pdf",
    pdfUploadedAt: "2026-06-24",
    rateValidationStatus: "pdf_uploaded",
    note: "Dokumentidentiteten er hash-verificeret mod 3F's aftale for murer- og murerarbejdsmandsarbejde 2025-2028. Geografiske undtagelser og alle satser kræver stadig manuel validering.",
  },
  {
    id: "isoleringsoverenskomsten",
    name: "Isoleringsoverenskomsten",
    category: "construction",
    industryArea: "Isolering / byggeri",
    isActive: true,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl: "",
    pdfFileName: "isoleringsoverenskomsten.pdf",
    pdfUploadedAt: "2026-06-24",
    rateValidationStatus: "pdf_uploaded",
    note: localArchiveNote,
  },
  {
    id: "maleroverenskomsten",
    name: "Maleroverenskomsten",
    category: "construction",
    industryArea: "Maler / byggeri / håndværk",
    isActive: true,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl: "",
    pdfFileName: "maleroverenskomsten.pdf",
    pdfUploadedAt: "2026-06-24",
    rateValidationStatus: "pdf_uploaded",
    note: "Den private arkivkopi kan ikke sikkert identificeres som en af de tre krævede maleraftaler. Kræver manuel validering.",
  },
  {
    id: "elektrikeroverenskomsten",
    name: "Elektrikeroverenskomsten",
    category: "electrical",
    industryArea: "El / installation",
    isActive: true,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl: "https://www.tekniq.dk/media/pwjbybrp/elektrikeroverenskomst-2025-2028.pdf",
    pdfFileName: "elektrikeroverenskomsten.pdf",
    pdfUploadedAt: "2026-06-24",
    rateValidationStatus: "pdf_uploaded",
    note: "Dokumentidentiteten er hash-verificeret mod TEKNIQ's Elektrikeroverenskomst 2025-2028. Ingen satser eller tillæg er valideret til automatisk beregning.",
  },
  {
    id: "el-overenskomsten-di-def",
    name: "El-overenskomsten (DI / DEF)",
    category: "electrical",
    industryArea: "El / industri / installation",
    isActive: true,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl: "",
    pdfFileName: "el-overenskomsten-di-def.pdf",
    pdfUploadedAt: "2026-06-24",
    rateValidationStatus: "pdf_uploaded",
    note: localArchiveNote,
  },
  {
    id: "vvs-overenskomsten",
    name: "VVS-overenskomsten",
    category: "plumbing",
    industryArea: "VVS",
    isActive: true,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl: "",
    pdfFileName: "vvs-overenskomsten.pdf",
    pdfUploadedAt: "2026-06-24",
    rateValidationStatus: "pdf_uploaded",
    note: "Den private arkivkopi kan ikke entydigt knyttes til den krævede VVS-aftale. Kræver manuel validering.",
  },
  {
    id: "industri-vvs-overenskomsten",
    name: "Industri- og VVS-overenskomsten",
    category: "plumbing",
    industryArea: "Industri / VVS",
    isActive: true,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl: "",
    pdfFileName: "industri-vvs-overenskomsten.pdf",
    pdfUploadedAt: "2026-06-24",
    rateValidationStatus: "pdf_uploaded",
    note: localArchiveNote,
  },
  {
    id: "vvs-blikkenslageroverenskomsten",
    name: "VVS- og Blikkenslageroverenskomsten",
    category: "plumbing",
    industryArea: "VVS / blikkenslager",
    isActive: true,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl: "",
    pdfFileName: "vvs-blikkenslageroverenskomsten.pdf",
    pdfUploadedAt: "2026-06-24",
    rateValidationStatus: "pdf_uploaded",
    note: "Den private arkivkopi kan ikke entydigt knyttes til Metal-, Blik- og Rør- eller VVS-aftalen. Kræver manuel validering.",
  },
  {
    id: "industriens-funktionaeroverenskomst",
    name: "Industriens Funktionæroverenskomst",
    category: "inactive",
    industryArea: "Funktionær / industri",
    isActive: false,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl: "",
    pdfFileName: "",
    pdfUploadedAt: "",
    rateValidationStatus: "missing_pdf",
    note: "Oprettet til senere brug. Skal ikke vises i version 1.",
  },
  {
    id: "hk-industrioverenskomsten",
    name: "HK-industrioverenskomsten",
    category: "inactive",
    industryArea: "HK / industri",
    isActive: false,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl: "",
    pdfFileName: "",
    pdfUploadedAt: "",
    rateValidationStatus: "missing_pdf",
    note: "Oprettet til senere brug. Skal ikke vises i version 1.",
  },
  {
    id: "hk-installationsoverenskomsten",
    name: "HK-installationsoverenskomsten",
    category: "inactive",
    industryArea: "HK / installation",
    isActive: false,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl: "",
    pdfFileName: "",
    pdfUploadedAt: "",
    rateValidationStatus: "missing_pdf",
    note: "Oprettet til senere brug. Skal ikke vises i version 1.",
  },
  {
    id: "tl-overenskomsten",
    name: "TL-overenskomsten",
    category: "inactive",
    industryArea: "Teknisk Landsforbund",
    isActive: false,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl: "",
    pdfFileName: "",
    pdfUploadedAt: "",
    rateValidationStatus: "missing_pdf",
    note: "Oprettet til senere brug. Skal ikke vises i version 1.",
  },
  {
    id: "auto-boligmonteringsoverenskomsten",
    name: "Auto- og Boligmonteringsoverenskomsten",
    category: "inactive",
    industryArea: "Auto / boligmontering",
    isActive: false,
    supportsLocalAgreement: true,
    requiresUploadedAgreementPdf: true,
    pdfUrl: "",
    pdfFileName: "",
    pdfUploadedAt: "",
    rateValidationStatus: "missing_pdf",
    note: "Oprettet til senere brug. Skal ikke vises i version 1.",
  },
];

export const collectiveAgreements = allCollectiveAgreements.filter((agreement) =>
  isSupportedCollectiveAgreementId(agreement.id),
);

export const activeCollectiveAgreements = collectiveAgreements.filter(
  (agreement) => agreement.isActive,
);

export function getCollectiveAgreementById(id: string) {
  return collectiveAgreements.find((agreement) => agreement.id === id);
}

export function getCollectiveAgreementByName(name: string) {
  return collectiveAgreements.find((agreement) => agreement.name === name);
}

export function publicAgreementPdfHref(pdfUrl?: string) {
  if (!pdfUrl) return "";
  if (/^https?:\/\//.test(pdfUrl)) return pdfUrl;
  const base = import.meta.env.BASE_URL || "/";
  return `${base.replace(/\/$/, "")}/${pdfUrl.replace(/^\//, "")}`;
}
